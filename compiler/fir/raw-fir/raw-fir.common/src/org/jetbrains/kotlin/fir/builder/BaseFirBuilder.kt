...
abstract class BaseFirBuilder<T>(val baseSession: FirSession, val context: Context<T> = Context()) {
    ...
    protected fun FirRegularClass.initCompanionObjectSymbolAttr() {
        val companionObject = declarations.firstOrNull { it is FirRegularClass && it.isCompanion } as FirRegularClass?
        companionObjectSymbol = companionObject?.symbol
    }
